<?php
/**
 * Inline-styled Sustainability Invoice for email body
...
<div style="max-width:760px;margin:0 auto;background:#ffffff;border-radius:10px;border:1px solid #e5e7eb;overflow:hidden">
  <div style="padding:16px 20px;border-bottom:4px solid #10b981;display:flex;justify-content:space-between;align-items:flex-start">
    <div>
      <?php if (!empty($logoDataUri)): ?>
        <img src="<?= h($logoDataUri) ?>" alt="Company Logo" style="max-height:56px;display:block"/>
      <?php else: ?>
        <div style="width:160px;height:48px;background:#10b981;color:#fff;font-weight:700;display:flex;align-items:center;justify-content:center;border-radius:6px">LOGO</div>
      <?php endif; ?>
    </div>
  ...
  <div style="padding:14px 20px;color:#111827;font-family:Arial,Helvetica,sans-serif">
    ...
    <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:10px;font-size:13px">
      <div><strong>Seller ID:</strong> <?= h($invoice->seller_id ?? '-') ?></div>
      <div style="text-align:right">
        <div><strong>Invoice No:</strong> <?= h($invoice->invoice_number) ?></div>
        <div><strong>Buyer ID:</strong> <?= h($invoice->buyer_id ?? '-') ?></div>
      </div>
    </div>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;font-size:12px">
      <thead>
        <tr>
          <th style="border:2px solid #0c5343;background:#0c5343;color:#fff;padding:8px;text-transform:uppercase">Quantity (MT)</th>
          <th style="border:2px solid #0c5343;background:#0c5343;color:#fff;padding:8px;text-transform:uppercase">Investment (USD)</th>
          <th style="border:2px solid #0c5343;background:#0c5343;color:#fff;padding:8px;text-transform:uppercase">Differential (USD)</th>
          <th style="border:2px solid #0c5343;background:#0c5343;color:#fff;padding:8px;text-transform:uppercase">Total Value (USD)</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td style="border:2px solid #0c5343;padding:8px;font-weight:700;text-align:center"><?= $this->Number->format($invoice->quantity_mt ?? 0, ['places' => 3]) ?></td>
          <td style="border:2px solid #0c5343;padding:8px;font-weight:700;text-align:center">$<?= number_format($invoice->sustainability_investment ?? 0, 2) ?></td>
          <td style="border:2px solid #0c5343;padding:8px;font-weight:700;text-align:center">$<?= number_format($invoice->sustainability_differential ?? 0, 2) ?></td>
          <td style="border:2px solid #0c5343;padding:8px;font-weight:700;text-align:center">$<?= number_format($invoice->total_value ?? 0, 2) ?></td>
        </tr>
      </tbody>
    </table>
