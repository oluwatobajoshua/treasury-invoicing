<?php
/**
 * PDF template for Sustainability Invoice
 * Vars: $invoice, $settings, $logoDataUri
 */
?>
<style>
.doc{max-width:900px;margin:0 auto;background:#fff;font-family:Arial,sans-serif}
.hdr{display:flex;justify-content:space-between;align-items:flex-start;padding:1rem 0 .5rem;border-bottom:4px solid #2e7d32}
...
.logo{width:180px;height:56px;background:#2e7d32;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700}
</style>
...
<div class="doc">
  ...
  <div class="title">SUSTAINABILITY INVOICE</div>
  ...
  <table class="tbl">
    ...
    <tbody>
      <tr>
        <td style="text-align:left;font-weight:600"><?= h($invoice->project_name ?? ($invoice->description ?? '-')) ?></td>
        <td><?= number_format($invoice->units ?? ($invoice->quantity ?? 0), 2) ?></td>
