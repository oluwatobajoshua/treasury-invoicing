 ...
 */
$this->assign('title', 'Clients Management');
?>
...
<div class="page-header-sleek">
    <div>
        ...
        <p class="page-subtitle-sleek">Manage client information and contacts</p>
    </div>
    <div>
        <?= $this->Html->link(
...
<div class="stats-row-sleek">
    <div class="stat-card-sleek">
        ...
        <div class="stat-label-sleek">Total Clients</div>
        <div class="stat-value-sleek"><?= count($clients) ?></div>
    </div>
    ...
    <div class="stat-card-sleek" style="border-left-color: var(--sunbeth-orange);">
        ...
        <div class="stat-label-sleek">With Email</div>
        <div class="stat-value-sleek">
            <?php 
            $emailCount = 0;
            foreach ($clients as $c) {
                if (!empty($c->email)) $emailCount++;
            }
            echo $emailCount;
            ?>
        </div>
    ...
    <div class="stat-card-sleek" style="border-left-color: #3b82f6;">
        ...
        <div class="stat-value-sleek">
            ...
            foreach ($clients as $c) {
                if ($c->created->format('Y-m') === $currentMonth) {
                    $thisMonth++;
                }
            }
            echo $thisMonth;
...
<div class="data-card-sleek">
    <div class="card-toolbar-sleek">
        ...
        <div class="toolbar-actions-sleek">
            <button class="btn-sleek btn-outline-sleek" onclick="$('#clientsTable').DataTable().button('.buttons-excel').trigger();">
                <i class="fas fa-file-excel"></i> Excel
            </button>
            <button class="btn-sleek btn-outline-sleek" onclick="$('#clientsTable').DataTable().button('.buttons-pdf').trigger();">
        ...
        <table id="clientsTable" class="table table-hover" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="product-name-sleek"><?= h($client->name) ?></div>
                    </td>
                    <td><?= h($client->contact_person) ?></td>
                    <td>
                        <?php if ($client->email): ?>
                            <a href="mailto:<?= h($client->email) ?>" style="color: var(--primary);">
                                <?= h($client->email) ?>
                            </a>
                        <?php else: ?>
                    ...
                    <td><?= h($client->created->format('M d, Y')) ?></td>
                    <td>
                        <div style="display: flex; gap: 0.375rem;">
                            ...
                            <?= $this->Form->postLink(
                                ...
                                [
                                    'confirm' => __('Are you sure you want to delete {0}?', $client->name),
                                    'class' => 'btn-action-sleek btn-delete-sleek',
                                    'escape' => false,
                                    'title' => 'Delete',
                                    'data-product-name' => $client->name,
