 ...
 * @var iterable<\App\Model\Entity\Vessel> $vessels
 */
$this->assign('title', 'Vessels Management');
?>
...
<div class="page-header-sleek">
    <div>
        ...
        <p class="page-subtitle-sleek">Manage vessel fleet and information</p>
    </div>
    <div>
        <?= $this->Html->link(
...
<div class="stats-row-sleek">
    <div class="stat-card-sleek">
        ...
        <div class="stat-label-sleek">Total Vessels</div>
        <div class="stat-value-sleek"><?= count($vessels) ?></div>
    </div>
    ...
    <div class="stat-card-sleek" style="border-left-color: var(--sunbeth-orange);">
        ...
        <div class="stat-label-sleek">With IMO</div>
        <div class="stat-value-sleek">
            <?php 
            $imoCount = 0;
            foreach ($vessels as $v) {
                if (!empty($v->imo_number)) $imoCount++;
            }
            echo $imoCount;
            ?>
        </div>
    ...
    <div class="stat-card-sleek" style="border-left-color: #3b82f6;">
        ...
        <div class="stat-value-sleek">
            ...
            foreach ($vessels as $v) {
                if ($v->created->format('Y-m') === $currentMonth) {
                    $thisMonth++;
                }
            }
            echo $thisMonth;
...
<div class="data-card-sleek">
    <div class="card-toolbar-sleek">
        <h3 class="toolbar-title-sleek">
            <i class="fas fa-list"></i> Vessel List
        </h3>
        <div class="toolbar-actions-sleek">
            <button class="btn-sleek btn-outline-sleek" onclick="$('#vesselsTable').DataTable().button('.buttons-excel').trigger();">
                <i class="fas fa-file-excel"></i> Excel
            </button>
            <button class="btn-sleek btn-outline-sleek" onclick="$('#vesselsTable').DataTable().button('.buttons-pdf').trigger();">
    ...
    <div class="card-body-sleek">
        <table id="vesselsTable" class="table table-hover" style="width:100%">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Vessel Name</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="product-name-sleek"><?= h($vessel->name) ?></div>
                    </td>
                    <td>
                        <?php if ($vessel->imo_number): ?>
                            <span class="product-code-sleek"><?= h($vessel->imo_number) ?></span>
                        <?php else: ?>
                    ...
                    <td><?= h($vessel->created->format('M d, Y')) ?></td>
                    <td>
                        <div style="display: flex; gap: 0.375rem;">
                            <?= $this->Html->link(
                                '<i class="fas fa-eye"></i>',
                                ['action' => 'view', $vessel->id],
                                ['class' => 'btn-action-sleek btn-view-sleek', 'escape' => false, 'title' => 'View']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-edit"></i>',
                                ['action' => 'edit', $vessel->id],
                                ['class' => 'btn-action-sleek btn-edit-sleek', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Form->postLink(
                                '<i class="fas fa-trash"></i>',
                                ['action' => 'delete', $vessel->id],
                                [
                                    'confirm' => __('Are you sure you want to delete {0}?', $vessel->name),
                                    'class' => 'btn-action-sleek btn-delete-sleek',
                                    'escape' => false,
                                    'title' => 'Delete',
                                    'data-product-name' => $vessel->name,
...
$(document).ready(function() {
    $('#vesselsTable').DataTable({
        ...
        language: {
            ...
            info: "Showing _START_ to _END_ of _TOTAL_ vessels",
            zeroRecords: "No matching vessels found"
        }
    });
});
