 ...
 */
$this->assign('title', 'Vessel Details');
?>
...
<div class="page-header-sleek">
  <div style="display:flex;justify-content:space-between;align-items:center">
    <h1><i class="fas fa-ship"></i> Vessel</h1>
    <div class="btn-group-sleek">
      <?= $this->Html->link('<i class="fas fa-arrow-left"></i> Back', ['action' => 'index'], ['class' => 'btn btn-light','escape' => false]) ?>
      <?= $this->Html->link('<i class="fas fa-edit"></i> Edit', ['action' => 'edit', $vessel->id], ['class' => 'btn btn-warning','escape' => false]) ?>
      <?= $this->Form->postLink('<i class="fas fa-trash"></i> Delete', ['action' => 'delete', $vessel->id], [
          'class' => 'btn btn-danger js-swal-post','escape' => false,
          'data-swal-title' => 'Delete this vessel?','data-swal-text' => 'This action cannot be undone.'
      ]) ?>
    </div>
  </div>
</div>
...
<div class="data-card">
  ...
  <div class="data-card-body">
    <div class="detail-row">
      ...
      <div><?= h($vessel->name) ?></div>
    </div>
    <div class="detail-row">
      <div class="detail-label">Flag Country</div>
      <div><?= h($vessel->flag_country ?? '') ?></div>
    </div>
    <div class="detail-row">
      <div class="detail-label">DWT (tons)</div>
      <div><?= h($vessel->dwt ?? '') ?></div>
    </div>
    <div class="detail-row">
      <div class="detail-label">Created</div>
      <div><?= $vessel->created ? $vessel->created->format('M d, Y H:i') : '' ?></div>
    </div>
    <div class="detail-row">
      <div class="detail-label">Modified</div>
      <div><?= $vessel->modified ? $vessel->modified->format('M d, Y H:i') : '' ?></div>
    </div>
  </div>
</div>
