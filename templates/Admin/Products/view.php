 ...
 */
$this->assign('title', 'Product Details');
?>
...
<div class="page-header-sleek">
  <div style="display:flex;justify-content:space-between;align-items:center">
    <h1><i class="fas fa-box"></i> Product</h1>
    <div class="btn-group-sleek">
      <?= $this->Html->link('<i class="fas fa-arrow-left"></i> Back', ['action' => 'index'], ['class' => 'btn btn-light','escape' => false]) ?>
      <?= $this->Html->link('<i class="fas fa-edit"></i> Edit', ['action' => 'edit', $product->id], ['class' => 'btn btn-warning','escape' => false]) ?>
      <?= $this->Form->postLink('<i class="fas fa-trash"></i> Delete', ['action' => 'delete', $product->id], [
          'class' => 'btn btn-danger js-swal-post','escape' => false,
          'data-swal-title' => 'Delete this product?','data-swal-text' => 'This action cannot be undone.'
      ]) ?>
    </div>
  </div>
</div>
...
<div class="data-card">
  ...
  <div class="data-card-body">
    ...
    <div class="detail-row">
      ...
      <div><?= h($product->category ?? '') ?></div>
    </div>
    <div class="detail-row">
      <div class="detail-label">Default Unit Price</div>
      <div><?= $this->Number->currency($product->unit_price ?? 0, 'USD') ?></div>
    </div>
    <div class="detail-row">
      <div class="detail-label">Created</div>
      <div><?= $product->created ? $product->created->format('M d, Y H:i') : '' ?></div>
    </div>
    <div class="detail-row">
      <div class="detail-label">Modified</div>
      <div><?= $product->modified ? $product->modified->format('M d, Y H:i') : '' ?></div>
    </div>
  </div>
</div>
