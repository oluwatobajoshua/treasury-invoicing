...
<div class="card fade-in" style="margin-bottom:.6rem;">
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.5rem;">
        <div>
            <h2 class="title" style="margin:0;font-size:1.05rem;">🏷️ Job Levels</h2>
            <p class="muted" style="margin:0;font-size:.7rem;">Maintain level codes and names</p>
        </div>
...
<table id="admin-joblevels-table" class="display responsive nowrap" style="width:100%">
    <thead>
        <tr>
            <th>ID</th>
            <th>Code</th>
...
<script>
$(function(){
    $('#admin-joblevels-table').DataTable({
        processing:true,
        serverSide:true,
        ajax:{ url:'<?= $this->Url->build(["action"=>"index"]) ?>', type:'GET', headers:{'X-Requested-With':'XMLHttpRequest'} },
        columns:[
            {data:'id', responsivePriority:2},
            {data:'level_code', responsivePriority:1, render:d=>'<code>'+d+'</code>'},
            {data:'level_name'},
            {data:'description', render:d=> d ? '<span class="muted">'+d+'</span>' : ''},
        ...
        language:{ search:'🔍 Search:', emptyTable:'No job levels found' }
    });
});
</script>
